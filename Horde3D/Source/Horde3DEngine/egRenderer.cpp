...
bool Renderer::init()
{
	...
	if( !_renderDevice->init() ) return false;
                           ...
                           int occSet )
{
	...
	for( size_t i = firstItem; i <= lastItem; ++i )
	{
		...
		if( occSet >= 0 )
		{
			...
			{
				queryObj = Modules::renderer().getRenderDevice()->createOcclusionQuery();
			...
			else
			{
				if( meshNode->_lastVisited[occSet] != Modules::renderer().getFrameID() )
				{
					...
					if( nearestDistToAABB( frust1->getOrigin(), meshNode->getBBox().min,
						...
						Modules::renderer().getRenderDevice()->getQueryResult( meshNode->_occQueries[occSet] ) < 1 )
					{
		...
		if( curGeoRes != modelNode->getGeometryResource() )
		{
			...
			// Indices
			Modules::renderer().getRenderDevice()->setIndexBuffer( curGeoRes->getIndexBuf(),
			...
			Modules::renderer().getRenderDevice()->setVertexBuffer( 0, posVBuf, 0, sizeof( Vec3f ) );
			Modules::renderer().getRenderDevice()->setVertexBuffer( 1, tanVBuf, 0, sizeof( VertexDataTan ) );
			Modules::renderer().getRenderDevice()->setVertexBuffer( 2, tanVBuf, sizeof( Vec3f ), sizeof( VertexDataTan ) );
			Modules::renderer().getRenderDevice()->setVertexBuffer( 3, staticVBuf, 0, sizeof( VertexDataStatic ) );
		}

		Modules::renderer().getRenderDevice()->setVertexLayout( Modules::renderer()._vlModel );
		...
		else
		{
			...
			Modules::renderer().getRenderDevice()->setShaderConst( Modules::renderer()._defColShader_color, CONST_FLOAT4, &color.x );
		}
		...
		if( modelChanged || curShader != prevShader )
		{
			...
			if( curShader->uni_skinMatRows >= 0 && !modelNode->_skinMatRows.empty() )
			{
				...
				Modules::renderer().getRenderDevice()->setShaderConst( curShader->uni_skinMatRows, CONST_FLOAT4,
		...
		{
			Modules::renderer().getRenderDevice()->setShaderConst( curShader->uni_worldMat, CONST_FLOAT44, &meshNode->_absTrans.x[0] );
		}
		...
		{
			Modules::renderer().getRenderDevice()->setShaderConst( curShader->uni_customInstData, CONST_FLOAT4,
			                                                       &modelNode->_customInstData[0].x, ModelCustomVecCount );
		}

		if( queryObj )
			Modules::renderer().getRenderDevice()->beginQuery( queryObj );
		
		// Render
		Modules::renderer().getRenderDevice()->drawIndexed( PRIM_TRILIST, meshNode->getBatchStart(), meshNode->getBatchCount(),
		...
		if( queryObj )
			Modules::renderer().getRenderDevice()->endQuery( queryObj );
	}
                              ...
                              int occSet )
{
	...
	Modules::renderer().getRenderDevice()->setVertexBuffer( 0, Modules::renderer().getParticleVBO(), 0, sizeof( ParticleVert ) );
	Modules::renderer().getRenderDevice()->setIndexBuffer( Modules::renderer().getQuadIdxBuf(), IDXFMT_16 );
	...
	for( uint32 i = firstItem; i <= lastItem; ++i )
	{
			...
			else
			{
				if( emitter->_lastVisited[occSet] != Modules::renderer().getFrameID() )
				{
					...
					if( nearestDistToAABB( frust1->getOrigin(), emitter->getBBox().min,
						...
						Modules::renderer().getRenderDevice()->getQueryResult( emitter->_occQueries[occSet] ) < 1 )
					{
		...
		// Set vertex layout
		Modules::renderer().getRenderDevice()->setVertexLayout( Modules::renderer()._vlParticle );
		...
		for( uint32 j = 0; j < emitter->_particleCount / ParticlesPerBatch; ++j )
		{
			...
			if( curShader->uni_parPosArray >= 0 )
				Modules::renderer().getRenderDevice()->setShaderConst( curShader->uni_parPosArray, CONST_FLOAT3,
				                                                       (float *)emitter->_parPositions + j*ParticlesPerBatch*3, ParticlesPerBatch );
			if( curShader->uni_parSizeAndRotArray >= 0 )
				Modules::renderer().getRenderDevice()->setShaderConst( curShader->uni_parSizeAndRotArray, CONST_FLOAT2,
				                                                       (float *)emitter->_parSizesANDRotations + j*ParticlesPerBatch*2, ParticlesPerBatch );
			if( curShader->uni_parColorArray >= 0 )
				Modules::renderer().getRenderDevice()->setShaderConst( curShader->uni_parColorArray, CONST_FLOAT4,
				                                                       (float *)emitter->_parColors + j*ParticlesPerBatch*4, ParticlesPerBatch );

			Modules::renderer().getRenderDevice()->drawIndexed( PRIM_TRILIST, 0, ParticlesPerBatch * 6, 0, ParticlesPerBatch * 4 );
			Modules::stats().incStat( EngineStats::BatchCount, 1 );
		...
		if( count > 0 )
		{
			...
			if( !allDead )
			{
				...
				Modules::renderer().getRenderDevice()->drawIndexed( PRIM_TRILIST, 0, count * 6, 0, count * 4 );
				Modules::stats().incStat( EngineStats::BatchCount, 1 );
