...
Resource *GeometryResource::clone()
{
	...
	*res = *this;
	...
	res->_indexBuf = Modules::renderer().getRenderDevice()->createIndexBuffer( _indexCount * (_16BitIndices ? 2 : 4), _indexData );
	res->_posVBuf = Modules::renderer().getRenderDevice()->createVertexBuffer( _vertCount * sizeof( Vec3f ), _vertPosData );
	res->_tanVBuf = Modules::renderer().getRenderDevice()->createVertexBuffer( _vertCount * sizeof( VertexDataTan ), _vertTanData );
	res->_staticVBuf = Modules::renderer().getRenderDevice()->createVertexBuffer( _vertCount * sizeof( VertexDataStatic ), _vertStaticData );
...
void GeometryResource::release()
{
	...
	{
		Modules::renderer().getRenderDevice()->destroyBuffer( _posVBuf );
		_posVBuf = 0;
	}
	if( _tanVBuf != 0 && _tanVBuf != defVertBuffer )
	{
		Modules::renderer().getRenderDevice()->destroyBuffer( _tanVBuf );
		_tanVBuf = 0;
	}
	if( _staticVBuf != 0 && _staticVBuf != defVertBuffer )
	{
		Modules::renderer().getRenderDevice()->destroyBuffer( _staticVBuf );
		_staticVBuf = 0;
	}
	
	if( _indexBuf != 0 && _indexBuf != defIndexBuffer )
	{
		Modules::renderer().getRenderDevice()->destroyBuffer( _indexBuf );
		_indexBuf = 0;
	}
...
bool GeometryResource::load( const char *data, int size )
{
	...
	{
		// Upload indices
		_indexBuf = Modules::renderer().getRenderDevice()->createIndexBuffer( _indexCount * (_16BitIndices ? 2 : 4), _indexData );
		
		// Upload vertices
		_posVBuf = Modules::renderer().getRenderDevice()->createVertexBuffer(_vertCount * sizeof( Vec3f ), _vertPosData );
		_tanVBuf = Modules::renderer().getRenderDevice()->createVertexBuffer( _vertCount * sizeof( VertexDataTan ), _vertTanData );
		_staticVBuf = Modules::renderer().getRenderDevice()->createVertexBuffer( _vertCount * sizeof( VertexDataStatic ), _vertStaticData );
	}
...
void GeometryResource::unmapStream()
{
	if( mappedWriteStream )
	{
		...
		case GeometryResData::GeoIndexStream:
			if( _indexData != 0x0 )
				Modules::renderer().getRenderDevice()->updateBufferData( _indexBuf, 0, _indexCount * (_16BitIndices ? 2 : 4), _indexData );
			break;
		case GeometryResData::GeoVertPosStream:
			if( _vertPosData != 0x0 )
				Modules::renderer().getRenderDevice()->updateBufferData( _posVBuf, 0, _vertCount * sizeof( Vec3f ), _vertPosData );
			break;
		case GeometryResData::GeoVertTanStream:
			if( _vertTanData != 0x0 )
				Modules::renderer().getRenderDevice()->updateBufferData( _tanVBuf, 0, _vertCount * sizeof( VertexDataTan ), _vertTanData );
			break;
		case GeometryResData::GeoVertStaticStream:
			if( _vertStaticData != 0x0 )
				Modules::renderer().getRenderDevice()->updateBufferData( _staticVBuf, 0, _vertCount * sizeof( VertexDataStatic ), _vertStaticData );
			break;
		}
