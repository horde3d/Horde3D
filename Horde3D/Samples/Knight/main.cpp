...
void keyPressListener(GLFWwindow* windowFromGLFW, int key, int scancode,int action,int modBits)
{
	...
	if( action == GLFW_PRESS )
	{
		...
		case GLFW_KEY_F1:
			...
			if( fullScreen )
			{
				windowFromGLFW = glfwCreateWindow(640, 480, "Knight", glfwGetPrimaryMonitor(), NULL);
...
bool setupWindow( int width, int height, bool fullscreen )
{
    ...
    windowFromGLFW = glfwCreateWindow(640, 480, "Knight", fullscreen ?glfwGetPrimaryMonitor():NULL, NULL);
...
int main( int argc, char** argv )
{
	// Initialize GLFW
	glfwInit();
    windowFromGLFW = glfwCreateWindow(640, 480, "inicial", NULL, NULL);
